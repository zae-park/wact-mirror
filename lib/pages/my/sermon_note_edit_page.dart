...
class _SermonNoteEditPageState extends State<SermonNoteEditPage> {
  ...
  void initState() {
    ...
    isChecked = widget.post['notice'];
    debugPrint('공지 여부: $isChecked');
  ...
  Future<void> _updatePost() async {
    ...
    // 공지 체크 여부 확인 및 추가
    if (isChecked != widget.post['notice']) {
      updateData['notice'] = isChecked; // 공지 여부 추가
      shouldUpdate = true;
    }
    ...
    if (shouldUpdate) {
      ...
      if (widget.post['notice'] == true && !isChecked) {
        // 공지에서 일반 글로 변경된 경우
