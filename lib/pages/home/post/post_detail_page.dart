...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  Future<void> toggleDisabledStatus() async {
    try {
      final updatedStatus = !(widget.post['disabled'] ?? false); // 현재 상태를 반전
      final response = await Supabase.instance.client
          .from('posts')
          .update({'disabled': updatedStatus}).eq('id', widget.post['id']);

      print('게시글 상태 업데이트: $response');

      setState(() {
        widget.post['disabled'] = updatedStatus;
      });
    } catch (e) {
      print('상태 업데이트 오류: $e');
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          toolbarHeight: 60,
          backgroundColor: Colors.white,
          title: (widget.post['notice'] == false)
              ? Text('')
              : Text(
                  '공지',
                  style: TextStyle(
          ...
          actions: [
            Row(
              children: [
                isAdmin && (widget.post['notice'] == false)
                    ? Padding(
                        ...
                        child: IconButton(
                          icon: Row(
                            children: [
                              Icon(
                                widget.post['disabled'] == true
                                    ? Icons.lock // 숨김 상태일 때 아이콘
                                    : Icons.lock_open, // 표시 상태일 때 아이콘
                                color: widget.post['disabled'] == true
                                    ? Colors.red
                                    : Colors.green,
                              ),
                              const SizedBox(width: 4), // 아이콘과 텍스트 간격
                              Text(
                                widget.post['disabled'] == true ? '숨김' : '표시',
                                style: TextStyle(
                                  color: widget.post['disabled'] == true
                                      ? Colors.red
                                      : Colors.green,
                          ...
                          onPressed: toggleDisabledStatus,
                        ),
                      )
                    : Container(),
                // if (widget.post['notice'] == true)
                          ...
                          if (value == 'edit') {
                            ...
                            if (result != null &&
                              ...
                              setState(() {
                                ...
                                if (result
                                    .containsKey('compressed_image_urls')) {
          ...
          children: [
            Padding(
              ...
              child: Row(
                ...
                children: [
                  SizedBox(
                    width: (widget.post['notice'] == false)
                        ? (MediaQuery.of(context).size.width * 2 / 3) - 15
                        : (MediaQuery.of(context).size.width * 2 / 3) - 5 - 20,
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
            ...
            Column(
              children: List.generate(
                ...
                (index) {
                  ...
                  bool isPostAuthor =
                      comment['author_id'] == widget.post['author_id'];
